...
def procesar_tax_767(detalle, cuit_agente, periodo):
    # Abrir el archivo de datos
    ddjj = file_utils.open_sicore_txt_file('767', cuit_agente, periodo)
    ...
    for index, row in tqdm(ddjj.iterrows(), total=ddjj.shape[0],
                           desc=f'Validando AFIP IVA (767) {cuit_agente} {periodo}'):
...
def procesar_fila(row, detalle):
    ...
    tax_condition = resultado['TaxCondition1'].iloc[0] if not resultado.empty else None
    suma = round(resultado['TaxCollectionAmount1'].sum(), 2)
...
def guardar_en_excel(ddjj, cuit_agente, periodo):
    with pd.ExcelWriter(f'validaciones/{cuit_agente} {periodo} reporte.xlsx', mode='a', engine='openpyxl') as writer:
        ddjj.to_excel(writer, sheet_name='tax_767', index=False)
